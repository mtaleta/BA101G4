...
<jsp:useBean id="listLaunchedProductsByCate_idDesc" scope="request" type="java.util.Set" />
...
<html lang="">
	<head>
		...
		<title>購物 - ${cateSvc.getOneCategory(param.cate_id).cate_name}</title>
	...
	<body>
		...
		<div class="container">
			...
			<div class="col-xs-12">
				<ul class="nav nav-pills">
					...
					<c:forEach var="cateVO" items="${cateSvc.getByProd_category(1)}" >
						<li ${(param.cate_id == cateVO.cate_id) ? "class=\"active\"" : ""}><a href="<%=request.getContextPath()%>/frontend/category/category.do?action=listLaunchedProductsByCate_idDesc&cate_id=${cateVO.cate_id}">${cateVO.cate_name}</a></li>
					</c:forEach>
				</ul>
			</div>
        	<hr class="col-xs-12">
			<div class="section-title">${cateSvc.getOneCategory(param.cate_id).cate_name}</div>
	        <hr class="col-xs-12">
			<%-- 錯誤表列 --%>
			...
			<%@ include file="/frontend/category/pages/page1.file" %> 
			<c:forEach var="productVO" items="${listLaunchedProductsByCate_idDesc}" varStatus="count" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
			...
			<%@ include file="/frontend/category/pages/page2.file" %>
			<br><br>
		</div><!-- container end -->
	</body>
