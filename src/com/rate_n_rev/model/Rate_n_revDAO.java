...
public class Rate_n_revDAO implements Rate_n_revDAO_interface {
	...
	// 晨均 晨均 晨均
	// 晨均 晨均 晨均
	private static final String GET_ALLRATE_BYSTORE_ID = "SELECT RNR_ID,MEM_ID,STORE_ID,RNR_RATE,RNR_REV,RNR_DATE FROM RATE_N_REV WHERE STORE_ID = ? order by RNR_DATE DESC";
	...
	public Set<Rate_n_revVO> getAllRate_n_rev_ByStore_id(String store_id){
		...
		Set<Rate_n_revVO> rate_n_revSet = new LinkedHashSet<Rate_n_revVO>();
		...
		try {
			...
			pstmt = con.prepareStatement(GET_ALLRATE_BYSTORE_ID);
			...
			while (rs.next()) {
				...
				rate_n_revSet.add(rate_n_revVO); // Store the row in the list
			}
