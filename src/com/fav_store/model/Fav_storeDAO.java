...
public class Fav_storeDAO implements Fav_storeDAO_interface {
	...
	// 晨均 晨均 晨均
	// 晨均 晨均 晨均
	private static final String GET_ALLFAVSTORE_BY_MEMID = "SELECT MEM_ID,STORE_ID FROM FAV_STORE WHERE MEM_ID = ? ";
	...
	public Set<Fav_storeVO> getAllFavStoreByMem_id(String mem_id) {
		...
		try {
			...
			pstmt = con.prepareStatement(GET_ALLFAVSTORE_BY_MEMID);
			...
			while (rs.next()) {
				...
				set.add(fav_storeVO); // Store the row in the list
			}
